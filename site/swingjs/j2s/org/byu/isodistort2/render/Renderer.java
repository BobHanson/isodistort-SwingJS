...
public class Renderer {
	 ...
	 * frontmost object at any position (x, y) in the image.
	 * 
...
//--- PUBLIC METHODS
	...
	/**
	 * Moves an already defined light i, to point in the new direction of normalized
	 * [x, y, z].
	 * 
	...
	/**
	 * flag that enables full manual camera control for setting camera location, aim
	 * target, and the up vector. (default = false). When true you can use :
	...
	/**
	 * Sets the camera to move to the eye position, aim at the center, and maintain
	 * the up direction ( requires the {@link #manualCameraControl} flag to be
	 * turned on).
	 * 
...
//temporary computed vertex
	...
	private void renderGeometry(Geometry s) {
		...
		if (!renderT && transparencyOf(s) != 0 && tS.length != 0) {
			try {
				tS[nt++] = s;
				return;
			} catch (ArrayIndexOutOfBoundsException e1) {
				nt--;
				System.out.println("Out of Bounds at(" + tS.length + ", " + nt + ")");
				return;
			} catch (NullPointerException e2) {
				System.out.println("Null Pointer at (" + tS.length + ", " + nt + ")");
				return;
			}
		}
...
//ZBUFFER A TRIANGLE, INTERPOLATING RED,GREEN,BLUE	
	...
	/*
	 * THIS IS THE INNER-LOOP RENDERING ROUTINE. IT'S THE COMPUTATIONAL BOTTLENECK,
	 * BECAUSE IT NEEDS TO PROCEED PIXEL BY PIXEL.
	 * 
	 * BECAUSE JAVA ENFORCES BOUNDS CHECKING ON EVERY ARRAY ACCESS (WHICH SLOWS
	 * THINGS SIGNIFICANTLY), I'VE BROKEN THINGS OUT HERE INTO INDIVIDUAL VARIABLES
	...
	private void fillTrapezoid(int A[], int B[], int C[], int D[], Geometry s) {
		...
		if (itest == 100)
			System.out.println("R0 " + itest++ + " " + A[0] + " " + A[1] + " " + B[0] + " " + B[1]);
...
//	private static double v[] = { 0, 0, 0, 0, 0, 0 };
	...
	private synchronized void renderVertex(int ix, int iy, int iz, double x, double y, double z, double v[],
			Material m) {
