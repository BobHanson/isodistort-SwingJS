 ...
 * 
 * The parameters passed to each new element are atomic number and 0 for xray
 ...
 */
	...
	private static class Element {
		...
		public double[] getValue(boolean isXray) {
			return (isXray ? new double[] { xray, 0 } : new double[] { real, imaginary });
		}
	}
		...
		private static HashMap<String,Element> periodicTable = new HashMap<String,Element>();
		...
		public static double[] getScatteringFactor(String symbol, boolean isXray) {
			Element element = (Element)periodicTable.get(symbol.toLowerCase());
			return (element == null ? NO_SUCH_ELEMENT : element.getValue(isXray));
		}	
	}	
