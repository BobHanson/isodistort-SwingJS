...
{
	/** True when a sltider bar moves, false otherwise */
	...
	public int numBonds;
	/**Specifies which atoms bond together; [bond #][atom1, atom2] Indicies reference atomInfo (below)*/
	public int[][] whichAtomsBond;
	/**Total number of unique parent atom types. */
	public int numUniques;
	/**Integer index that identifies each parent atom type as one of several unique parent atom types. */
	public int[] atomTypeUnique;
	/**Array of colors for slider bars, they correspond to which atoms they move.*/
	public Color[] color;
	/**Array of colors for scalar slider bars, they correspond to which atoms they move.*/
	public Color[] scalarcolor;
	/**Array of colors for magnetic slider bars, they correspond to which moments they adjust.*/
	public Color[] magcolor;
	/**Array of colors for rotational slider bars, they correspond to which angles they adjust.*/
	public Color[] rotcolor;
	/**Array of colors for ellipsoidal slider bars, they correspond to which moments they adjust.*/
	public Color[] ellipcolor;
	/** If true (when at least two parents have the same element type) show the simple-color checkbox.*/
	...
	/**Parent cell origin relative to supercell origin on the unitless superlattice basis.  [x, y, z]*/
	public double[] pOriginUnitless = new double[3];
	/**Row matrix of parent basis vectors on the unitless superlattice basis [basis vector][x,y,z]*/
	...
	/**Strained parent cell origin relative to strained supercell origin in cartesian Angstrom coords.  [x, y, z]*/
	...
	/**Matrix of unstrained super basis vectors in cartesian Angstrom coords [basis vector][x,y,z]
	 * Transforms unitless unstrained direct-lattice supercell coords into cartesian Angstrom coords [basis vector][x,y,z]*/
	public double[][] sBasisCart0 = new double[3][3];
	/**Matrix of strained super basis vectors in cartesian Angstrom coords [basis vector][x,y,z]
	 * Transforms unitless strained direct-lattice supercell coords into cartesian Angstrom coords [basis vector][x,y,z]*/
	...
	/**Matrix of unstrained parent basis vectors in cartesian Angstrom coords [basis vector][x,y,z]
	 * Transforms unitless unstrained direct-lattice parentcell coords into cartesian Angstrom coords [basis vector][x,y,z]*/
	public double[][] pBasisCart0 = new double[3][3];
	/**Matrix of strained parent basis vectors in cartesian Angstrom coords [basis vector][x,y,z]
	 * Transforms unitless strained direct-lattice parentcell coords into cartesian Angstrom coords [basis vector][x,y,z]*/
	public double[][] pBasisCart = new double[3][3];
	/**Matrix of strained super-cell crystal-axis basis vectors in unitless coords [basis vector][x,y,z]
	 * Transforms unitless strained direct-lattice supercell crystalaxis coords into cartesian coords [basis vector][x,y,z]*/
	public double[][] sCrystalAxisBasisCart = new double[3][3];
	/**Array of vertices of strained window-centered parent cell.  [edge number][x, y, z]*/
	public double[][] parentCellVertices = new double[8][3];
	/**Array of vertices of strained window-centered super cell.  [edge number][x, y, z]*/
	...
	/**Array of atom positions in unitless lattice units. [type][subtype][atom of subtype][x, y, z]*/
	...
	/**Array of atom magnetic moment vectors in reciprocal lattice units (muB/Ang). [type][subtype][atom of subtype][mx, my, mz]*/
	public double[][][][] atomInitMag, atomFinalMag;
	/**Array of atom magnetic moment vectors in reciprocal lattice units (muB/Ang). [type][subtype][atom of subtype][mx, my, mz]*/
	public double[][][][] atomInitRot, atomFinalRot;
	/**Array of atom ellipsoid params in Angstrom-squared units. [type][subtype][atom of subtype][xx, yy, zz, yz, xz, xy]*/
	...
	/**Boolean variable that tracks whether irrep sliders were last set to sliderMax or to zero.*/
	...
	/**Array of displacement mode vectors. [type][mode of type][subtype][atom of subtype][x, y, z]*/
	...
	/**Array of scalar mode magnitudes. [type][mode of type][subtype][atom of subtype]*/
	...
	/**Array of magnetic mode vectors. [type][mode of type][subtype][atom of subtype][x, y, z]*/
	...
	/**Array of rotational mode vectors. [type][mode of type][subtype][atom of subtype][x, y, z]*/
	...
	/**Array of ellipsoidal mode vectors. [type][mode of type][subtype][atom of subtype][xx, yy, zz, yz, xz, xy]*/
...
//	Global variables that are part of the Panel.
	/**Master (top most) slider bar controls all slider bars for superpositioning of modes.*/
	...
	/**Array of labels for displacement mode slider bars.  Label specifies name of mode and mode bar displacement.*/
	public JLabel[][] dispmodeSliderLabel;
	/**Array of labels for scalar mode slider bars.  Label specifies name of mode and mode bar displacement.*/
	public JLabel[][] scalarmodeSliderLabel;
	/**Array of labels for magnetic mode slider bars.  Label specifies name of mode and mode bar displacement.*/
	public JLabel[][] magmodeSliderLabel;
	/**Array of labels for rotational mode slider bars.  Label specifies name of mode and mode bar displacement.*/
	public JLabel[][] rotmodeSliderLabel;
	/**Array of labels for ellipsoidal mode slider bars.  Label specifies name of mode and mode bar displacement.*/
	public JLabel[][] ellipmodeSliderLabel;
	/**Array of labels for strain mode slider bars.  Label specifies name of mode and mode bar displacement.*/
	public JLabel[] strainmodeSliderLabel;
	/**Array of labels for irrep mode slider bars.  Label specifies name of mode and mode bar displacement.*/
	...
	/**Holds all the slider bars but not the viewPanel above it; added to controlPanel*/
	...
	/**Panel which holds the occupancy check boxes for each atom subtype associated with a type*/
	...
	/**Pane that holds the control panels*/
	public JScrollPane controlPane;

	/**Maximum slider bar value.  The slider bar only takes integer values.  So the sliderMax
	 * is the number of notches and therefore the precision of the slider bar.  More notches
	 * make for more precise slider movements but slow the rendering a lot since it renders at every notch*/
	public int sliderMax = 100;
	/**double version of sliderMax*/
	public double sliderMaxVal = sliderMax;
	/** Applet width and height in pixels.  Rendering width and height in pixels */
	public int renderingWidth, renderingHeight;
...
//			System.out.println("Next token: "+next);
		...
		{
			currentData = (ArrayList<String>) myMap.get(currentTag);
		...
		if(myMap.containsKey(currentTag))
		{
			...
			{
				appletWidth = Integer.parseInt((String)currentData.get(0));
			}
		}
		else	// Set appletWidth to default value
			appletWidth = 1024;
...
//						subTypeName[t][s]=atomTypeName[t]+"_"+(s+1);
		...
		if(myMap.containsKey(currentTag))
		{
			currentData = (ArrayList<String>)myMap.get(currentTag);
			int dataPerRow = 6;
			numAtoms = currentData.size()/dataPerRow;
...
//				System.out.println("old atom coords");
			...
			else
			{
				...
				//	input the unitless atomic coords [type][subtype][atom number][x,y,z][occ][mx,my,mz]
				int q = 0;
				for (int t=0; t<numTypes; t++)//iterate over all the types
					for (int s=0; s<numSubTypes[t]; s++)
						...
						{
							for (int i=0; i<3; i++)
			 					atomInitCoord[t][s][a][i] = Double.parseDouble((String)currentData.get(q*dataPerRow+3+i));
							q++;
						}
			}
		}
		...
		if(myMap.containsKey(currentTag))
		{
			int dataPerRow = 1;
			currentData = (ArrayList<String>)myMap.get(currentTag);
			if(currentData.size() != numAtoms*dataPerRow)	// Make sure it's divided evenly and that numAtoms hasn't changed.
			{
				parseError(currentTag,1);
			}
			else
			...
			else
			{
				...
				for (int t=0; t<numTypes; t++)//iterate over all the types
					for (int s=0; s<numSubTypes[t]; s++)
						...
						{
							for (int i=0; i<3; i++)
			 					atomInitMag[t][s][a][i] = Double.parseDouble((String)currentData.get(q*dataPerRow+i));
		 					q++;
						}
			}
		}
			...
			else
			{
				...
				for (int t=0; t<numTypes; t++)//iterate over all the types
					for (int s=0; s<numSubTypes[t]; s++)
						...
						{
							for (int i=0; i<3; i++)
			 					atomInitRot[t][s][a][i] = Double.parseDouble((String)currentData.get(q*dataPerRow+i));
		 					q++;
						}
			}
		}
			...
			else
			{
				...
				for (int t=0; t<numTypes; t++)//iterate over all the types
					for (int s=0; s<numSubTypes[t]; s++)
						...
						{
							for (int i=0; i<6; i++)
			 					atomInitEllip[t][s][a][i] = Double.parseDouble((String)currentData.get(q*dataPerRow+i));
							q++;
						}
			}
		}
			...
			for (int b=0; b<numBonds; b++)//iterate over all the types
		  	{
		  		...
		  		for (int t=0; t<numTypes; t++)//iterate over all the types
		  			for (int s=0; s<numSubTypes[t]; s++)
		  				...
		  				{
							for (int p=0; p<2; p++)
								if ((detailedWhichAtomsBond[b][p][0]==t+1)&&(detailedWhichAtomsBond[b][p][1]==s+1)&&(detailedWhichAtomsBond[b][p][2]==a+1))
		...
		if(myMap.containsKey(currentTag))
		{
			...
			//	input displacement mode array [atom type][mode number of that type][atom number of that type][x,y,z]
			counter = 0;
			for(int m=0; m<dispmodeNum; m++)//iterate over modes
			{
				int thisType = Integer.parseInt((String)currentData.get(counter))-1;//first number is the type number
				counter++;
				...
				counter++;
				dispmodeInitAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//amplitude of mode
				counter++;
				dispmodeMaxAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//maximum amplitude of mode
				counter++;
				dispmodeIrrep[thisType][modeTracker[thisType]]=Integer.parseInt((String)currentData.get(counter))-1;//irrep of mode
				counter++;
				dispmodeName[thisType][modeTracker[thisType]] = (String)currentData.get(counter);//name of first mode is made of four chunks
				counter++;
				for (int s=0; s<numSubTypes[thisType]; s++)	
					for (int a=0; a<numSubAtoms[thisType][s];a++)//iterate through the atoms
						for (int i=0; i<3; i++)
						{
							dispmodeVect[thisType][modeTracker[thisType]][s][a][i]=Double.parseDouble((String)currentData.get(counter));
							counter++;
						}
		...
		if(myMap.containsKey(currentTag))
		{
			...
			for(int m=0; m<scalarmodeNum; m++)//iterate over modes
			{
				...
				counter++;
				scalarmodeInitAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//amplitude of mode
				counter++;
				scalarmodeMaxAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//maximum amplitude of mode
				counter++;
				scalarmodeIrrep[thisType][modeTracker[thisType]]=Integer.parseInt((String)currentData.get(counter))-1;//irrep of mode
				counter++;
				scalarmodeName[thisType][modeTracker[thisType]] = (String)currentData.get(counter);//name of first mode is made of four chunks
				counter++;
				for (int s=0; s<numSubTypes[thisType]; s++)	
					for (int a=0; a<numSubAtoms[thisType][s];a++)//iterate through the atoms
					{
						scalarmodeVect[thisType][modeTracker[thisType]][s][a]=Double.parseDouble((String)currentData.get(counter));
						counter++;
					}
				modeTracker[thisType] +=1;
			}
		...
		if(myMap.containsKey(currentTag))
		{
			...
			//	input magnetic mode array [atom type][mode number of that type][atom number of that type][mx,my,mz]
			counter = 0;
			for(int m=0; m<magmodeNum; m++)//iterate over modes
			{
				...
				counter++;
				magmodeInitAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//amplitude of mode
				counter++;
				magmodeMaxAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//maximum amplitude of mode
				counter++;
				magmodeIrrep[thisType][modeTracker[thisType]]=Integer.parseInt((String)currentData.get(counter))-1;//irrep of mode
				counter++;
				magmodeName[thisType][modeTracker[thisType]] = (String)currentData.get(counter);//name of first mode is made of four chunks
				counter++;
				for (int s=0; s<numSubTypes[thisType]; s++)	
					for (int a=0; a<numSubAtoms[thisType][s];a++)//iterate through the atoms
						for (int i=0; i<3; i++)
						{
							magmodeVect[thisType][modeTracker[thisType]][s][a][i]=Double.parseDouble((String)currentData.get(counter));
							counter++;
						}
				modeTracker[thisType] +=1;
			}
		...
		if(myMap.containsKey(currentTag))
		{
			...
			//	input rotational mode array [atom type][mode number of that type][atom number of that type][mx,my,mz]
			counter = 0;
			for(int m=0; m<rotmodeNum; m++)//iterate over modes
			{
				...
				counter++;
				rotmodeInitAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//amplitude of mode
				counter++;
				rotmodeMaxAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//maximum amplitude of mode
				counter++;
				rotmodeIrrep[thisType][modeTracker[thisType]]=Integer.parseInt((String)currentData.get(counter))-1;//irrep of mode
				counter++;
				rotmodeName[thisType][modeTracker[thisType]] = (String)currentData.get(counter);//name of first mode is made of four chunks
				counter++;
				for (int s=0; s<numSubTypes[thisType]; s++)	
					for (int a=0; a<numSubAtoms[thisType][s];a++)//iterate through the atoms
						for (int i=0; i<3; i++)
						{
							rotmodeVect[thisType][modeTracker[thisType]][s][a][i]=Double.parseDouble((String)currentData.get(counter));
							counter++;
						}
				modeTracker[thisType] +=1;
			}
...
//			System.out.println(t+" "+rotmodePerType[t]);
		...
		if(myMap.containsKey(currentTag))
		{
			...
			//	input ellptical mode array [atom type][mode number of that type][atom number of that type][xx,yy,zz,xy,xz,xy]
			counter = 0;
			for(int m=0; m<ellipmodeNum; m++)//iterate over modes
			{
				...
				counter++;
				ellipmodeInitAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//amplitude of mode
				counter++;
				ellipmodeMaxAmp[thisType][modeTracker[thisType]]=Double.parseDouble((String)currentData.get(counter));//maximum amplitude of mode
				counter++;
				ellipmodeIrrep[thisType][modeTracker[thisType]]=Integer.parseInt((String)currentData.get(counter))-1;//irrep of mode
				counter++;
				ellipmodeName[thisType][modeTracker[thisType]] = (String)currentData.get(counter);//name of first mode is made of four chunks
				counter++;
				for (int s=0; s<numSubTypes[thisType]; s++)	
					for (int a=0; a<numSubAtoms[thisType][s];a++)//iterate through the atoms
						for (int i=0; i<6; i++)
						{
							ellipmodeVect[thisType][modeTracker[thisType]][s][a][i]=Double.parseDouble((String)currentData.get(counter));
							counter++;
						}
				modeTracker[thisType] +=1;
			}
	...
	public void readSliders()
	{
		...
		masterSliderLabel.setText(mytoString(masterSliderVal,2,-8)+"  Master Slider");//change the master label to display the new amplitude value
...
//  Divide the applet area with structure on the left and controls on the right.
		...
		int controlPanelHeight = 55, roomForScrollBar = 15, padding = 4, barheight = 20;
		int subTypeWidth = 170, subTypeBoxWidth = 20, sliderLabelWidth = 36, lattWidth = 60;

		int controlPaneHeight = controlPanelHeight+padding;
		int scrollPaneHeight = appletHeight-controlPaneHeight;
		int scrollPanelHeight = scrollPaneHeight-padding;
		int controlPaneWidth = appletWidth;
		int controlPanelWidth = controlPaneWidth-padding;
		int scrollPaneWidth = appletWidth-scrollPaneHeight;		
		int scrollPanelWidth = scrollPaneWidth-roomForScrollBar-padding;
		int sliderWidth = scrollPanelWidth/2;

		renderingHeight = scrollPaneHeight;
		...
		int numExtraRows = 5; //One for master slider bar, one for strainTitle, one for irrepTitle, and 2 for the lattice params.
		/** Minimum number of rows in grid that will keep the rows thin */
		int minRowNumber = (int)Math.floor(scrollPanelHeight/barheight);

		/**Maximum number of check boxes per row the GUI will hold*/
		int maxSubTypesPerRow=(int)Math.floor(scrollPanelWidth/subTypeWidth);
		...
		/**Total number of rows needed for checkboxes*/
		...
		int rowCount = dispmodeNum+scalarmodeNum+magmodeNum+rotmodeNum+ellipmodeNum+strainmodeNum+numIrreps+numTypes+numExtraRows+numSubRowsTotal;
		int rowNumber = Math.max(rowCount,minRowNumber);
//		System.out.println("rowCount: "+rowCount+", rowNumber: "+rowNumber);
//		System.out.println("scrollPanelWidth="+scrollPanelWidth+", maxBoxesPerRow="+maxBoxesPerRow);

		
//		Initialize scrollPanel
		scrollPanel = new JPanel();
	    scrollPanel.setBackground(Color.WHITE);
		scrollPanel.setLayout(new FlowLayout(FlowLayout.LEFT,0,0)); //sets grid length equal to number of rows.
		scrollPanel.setPreferredSize(new Dimension(scrollPanelWidth,rowNumber*barheight));//size of master slider bar's label

		//Master Slider Panel
		masterSliderLabel = new JLabel();//label for master slider bar
		masterSliderLabel.setPreferredSize(new Dimension(sliderLabelWidth,barheight));//size of master slider bar's label
		...
		masterSlider.setBackground(new Color(255, 255, 255));//color of master slider bar
		...
		masterSliderPanel.setPreferredSize(new Dimension(scrollPanelWidth, barheight));
		...
		scrollPanel.add(masterSliderPanel);//add master slider bar to top of scrollPanel
		...
		for (int t=0; t<numTypes; t++)
		{
			...
			typeLabel[t].setPreferredSize(new Dimension(scrollPanelWidth, barheight));//size of slider labels
			...
			typeNamePanel[t].setPreferredSize(new Dimension(scrollPanelWidth, barheight));
...
//			System.out.println("t: "+t+", numSubTypes[t]: "+numSubTypes[t]+", boxesPerRow[t]: "+boxesPerRow[t]+", numCheckRows[t]: "+numCheckRows[t]);
			...
			scrollPanel.add(typeNamePanel[t]);

			//typeDataPanel
			typeDataPanel[t] = new JPanel();
			typeDataPanel[t].setPreferredSize(new Dimension(scrollPanelWidth, numSubRows[t]*barheight));
			...
			for (int s=numSubTypes[t]; s<numSubRows[t]*subTypesPerRow[t]; s++)
				typeDataPanel[t].add(new JLabel("")); // Makes sure that each subtype row is full for allignment purposes

			scrollPanel.add(typeDataPanel[t]);

			//dispPanel
			dispPanel[t] = new JPanel();
			dispPanel[t].setPreferredSize(new Dimension(scrollPanelWidth, dispmodePerType[t]*barheight));//size of slider labels
			...
			for (int m=0; m<dispmodePerType[t]; m++)//iterate through array of slider bars
			{
				...
				dispmodeSliderLabel[t][m].setPreferredSize(new Dimension(sliderLabelWidth, barheight));//size of slider labels
				...
				dispmodeSlider[t][m] = new JSlider(JSlider.HORIZONTAL, -(int) sliderMax, (int) sliderMax, (int)((dispmodeInitAmp[t][m]/dispmodeMaxAmp[t][m])*sliderMax));//put a slider bar in the ith slot
			...
			scrollPanel.add(dispPanel[t]);//add this modePanel to scrollPanel

			//scalarPanel
			scalarPanel[t] = new JPanel();
			scalarPanel[t].setPreferredSize(new Dimension(scrollPanelWidth, scalarmodePerType[t]*barheight));//size of slider labels
			...
			for (int m=0; m<scalarmodePerType[t]; m++)//iterate through array of slider bars
			{
				...
				scalarmodeSliderLabel[t][m].setPreferredSize(new Dimension(sliderLabelWidth, barheight));//size of slider labels
				...
				scalarmodeSlider[t][m] = new JSlider(JSlider.HORIZONTAL, -(int) sliderMax, (int) sliderMax, (int)((scalarmodeInitAmp[t][m]/scalarmodeMaxAmp[t][m])*sliderMax));//put a slider bar in the ith slot
			...
			scrollPanel.add(scalarPanel[t]);//add this modePanel to scrollPanel

			//magPanel
			magPanel[t] = new JPanel();
			magPanel[t].setPreferredSize(new Dimension(scrollPanelWidth, magmodePerType[t]*barheight));//size of slider labels
			...
			for (int m=0; m<magmodePerType[t]; m++)//iterate through array of slider bars
			{
				...
				magmodeSliderLabel[t][m].setPreferredSize(new Dimension(sliderLabelWidth, barheight));//size of slider labels
				...
				magmodeSlider[t][m] = new JSlider(JSlider.HORIZONTAL, -(int) sliderMax, (int) sliderMax, (int)((magmodeInitAmp[t][m]/magmodeMaxAmp[t][m])*sliderMax));//put a slider bar in the ith slot
			...
			scrollPanel.add(magPanel[t]);//add this modePanel to scrollPanel

			//rotPanel
			rotPanel[t] = new JPanel();
			rotPanel[t].setPreferredSize(new Dimension(scrollPanelWidth, rotmodePerType[t]*barheight));//size of slider labels
			...
			for (int m=0; m<rotmodePerType[t]; m++)//iterate through array of slider bars
			{
				...
				rotmodeSliderLabel[t][m].setPreferredSize(new Dimension(sliderLabelWidth, barheight));//size of slider labels
				...
				rotmodeSlider[t][m] = new JSlider(JSlider.HORIZONTAL, -(int) sliderMax, (int) sliderMax, (int)((rotmodeInitAmp[t][m]/rotmodeMaxAmp[t][m])*sliderMax));//put a slider bar in the ith slot
			...
			scrollPanel.add(rotPanel[t]);//add this modePanel to scrollPanel

			//ellipPanel
			ellipPanel[t] = new JPanel();
			ellipPanel[t].setPreferredSize(new Dimension(scrollPanelWidth, ellipmodePerType[t]*barheight));//size of slider labels
			...
			for (int m=0; m<ellipmodePerType[t]; m++)//iterate through array of slider bars
			{
				...
				ellipmodeSliderLabel[t][m].setPreferredSize(new Dimension(sliderLabelWidth, barheight));//size of slider labels
				...
				ellipmodeSlider[t][m] = new JSlider(JSlider.HORIZONTAL, -(int) sliderMax, (int) sliderMax, (int)((ellipmodeInitAmp[t][m]/ellipmodeMaxAmp[t][m])*sliderMax));//put a slider bar in the ith slot
			...
			scrollPanel.add(ellipPanel[t]);//add this modePanel to scrollPanel
		}
		
		//strainTitlePanel
		JLabel strainTitle = new JLabel("StrainModes");
		strainTitle.setPreferredSize(new Dimension(scrollPanelWidth, barheight));
		...
		strainTitlePanel.setPreferredSize(new Dimension(scrollPanelWidth, barheight));
		...
		scrollPanel.add(strainTitlePanel);
		...
		strainDataPanel.setPreferredSize(new Dimension(scrollPanelWidth, 2*barheight));
		...
		scrollPanel.add(strainDataPanel);
		
		//strainPanel
		strainPanel = new JPanel();
		strainPanel.setPreferredSize(new Dimension(scrollPanelWidth, strainmodeNum*barheight));//size of slider labels
		...
		for (int m=0; m<strainmodeNum; m++)//iterate through array of slider bars
		{
			...
			strainmodeSliderLabel[m].setPreferredSize(new Dimension(sliderLabelWidth, barheight));//size of slider labels
			...
			strainmodeSlider[m] = new JSlider(JSlider.HORIZONTAL, -(int) sliderMax, (int) sliderMax, (int)((strainmodeInitAmp[m]/strainmodeMaxAmp[m])*sliderMax));//put a slider bar in the ith slot
			...
			scrollPanel.add(strainPanel);//add this modePanel to scrollPanel
		}
		
		//irrepTitlePanel
		JLabel irrepTitle = new JLabel("Single-Irrep Master Amplitudes");
		irrepTitle.setPreferredSize(new Dimension(scrollPanelWidth, barheight));
		...
		irrepTitlePanel.setPreferredSize(new Dimension(scrollPanelWidth, barheight));
		...
		scrollPanel.add(irrepTitlePanel);

		//irrepPanel
		irrepPanel = new JPanel();
		irrepPanel.setPreferredSize(new Dimension(scrollPanelWidth, numIrreps*barheight));//size of slider labels
		...
		for (int m=0; m<numIrreps; m++)//iterate through array of slider bars
		{
			...
			irrepmodeSliderLabel[m].setPreferredSize(new Dimension(sliderLabelWidth, barheight));//size of slider labels
			...
			irrepmodeSlider[m] = new JSlider(JSlider.HORIZONTAL, 0, (int) sliderMax, sliderMax);//put a slider bar in the ith slot
			...
			scrollPanel.add(irrepPanel);//add this modePanel to scrollPanel
		}
		
		// Intialize the scrollPanel
		...
		controlPane.setBorder(BorderFactory.createLineBorder(Color.BLACK,1));
		controlPane.setPreferredSize(new Dimension(controlPaneWidth, controlPaneHeight));
	...
	public void setColors(boolean simpleColor)
	{
		...
		for (int t=0; t<numTypes; t++)//iterate over types of atoms
		{
			if (simpleColor)
				...
				k = 1.0*t/numTypes;
			color[t] = new Color(Color.HSBtoRGB((float) k, (float) 1.0, (float) 0.95));//makes a rainbow of equally spaced colors according to number of atoms
			scalarcolor[t] = new Color(Color.HSBtoRGB((float) k, (float) 1.0, (float) 0.80));//makes a rainbow of equally spaced colors according to number of atoms
			magcolor[t] = new Color(Color.HSBtoRGB((float) k, (float) 1.0, (float) 0.65));//makes a rainbow of equally spaced colors according to number of atoms
			rotcolor[t] = new Color(Color.HSBtoRGB((float) k, (float) 1.0, (float) 0.55));//makes a rainbow of equally spaced colors according to number of atoms
			ellipcolor[t] = new Color(Color.HSBtoRGB((float) k, (float) 1.0, (float) 0.40));//makes a rainbow of equally spaced colors according to number of atoms
		}
	}
